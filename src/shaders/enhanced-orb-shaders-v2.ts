...
export const getEnhancedFragShaderV2 = (octaves: number, useNoiseTexture: boolean = false) => `
precision highp float;
...
${useNoiseTexture ? `
// Use precomputed 3D noise texture for performance
uniform sampler3D uNoiseTexture;

float sampleNoise(vec3 p) {
  return texture(uNoiseTexture, p * 0.5 + 0.5).r * 2.0 - 1.0;
}
` : `
