...
export function Header() {
  ...
  return (
    <header
      className={clsx(
        "fixed top-0 inset-x-0 z-40 transition-all duration-1000 ease-in-out pointer-events-none",
        isActive ? "opacity-60" : "opacity-100"
      )}
    >
      <div className="mx-auto w-full max-w-[var(--ui-max-width)] px-[var(--ui-pad-x)] pt-[calc(var(--ui-pad-y)+env(safe-area-inset-top))] pb-[var(--ui-pad-y)] flex justify-between items-start">
