 ...
 * - Pattern variety
 * 
 * Adapted from Others/patternRecommender.ts for ZenB-1
 */
...
export function recommendPatterns(limit: number = 3): PatternRecommendation[] {
    ...
    const recentPatterns = history.slice(0, 5).map(s => s.patternId);
...
export function getPersonalizedGreeting(): string {
    ...
    if (userSettings.streak >= 7) {
        greeting += ` 🔥 ${userSettings.streak}-day streak!`;
    }
