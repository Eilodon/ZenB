...
export function calculateBreathScale(
  ...
  const { baseScale, maxScale, minScale, anticipationAmount, overshootAmount, holdOscillation } = cfg;
  ...
  switch (phase) {
    case 'inhale': {
      ...
      const easedProgress = BreathCurves.inhale(progress);

      // Anticipation at start (slight squash)
      if (progress < 0.12) {
  ...
  return Math.max(minScale, Math.min(maxScale + overshootAmount, scale));
}
